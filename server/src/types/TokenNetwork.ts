...
class TokenNetwork {
	...
	async getBlock(hash: string) {
		const block: AccountBlockBlock = await this.viteApi.request(
			//
...
const shiftDot = (num: string, decimals = 0) => {
	return decimals < 0
		? new Big(num).div(new Big(10).pow(-decimals))
		: new Big(num).times(new Big(10).pow(decimals));
};
