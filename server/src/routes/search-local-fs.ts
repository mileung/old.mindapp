...
const searchLocalFs: RequestHandler = (req, res) => {
  ...
  const spaceIds =
    typeof req.query.spaceIds === "string" ? req.query.spaceIds.split(",") : [];
  ...
  for (let i = 0; i < spacesDir.length; i++) {
    ...
    const periodDirs = fs
      .readdirSync(spaceIdPath)
      .sort((a, b) => (oldToNew ? +a - +b : +b - +a));
    ...
    for (let i = 0; i < periodDirs.length; i++) {
      ...
      for (let i = 0; i < dayDirs.length; i++) {
        ...
        for (let i = 0; i < notesDir.length; i++) {
          ...
          const noteTimestamp = Number(
            fileName.substring(0, fileName.indexOf("."))
          );
          // console.log("noteTimestamp:", noteTimestamp);
          if (
            oldToNew ? pageAfter >= noteTimestamp : pageAfter <= noteTimestamp
          )
            continue;
